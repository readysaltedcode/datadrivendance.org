...
datastorm.vis.forecast = (function(){
  ...
  var elapsedTs;
  var durationTs = 180000; // full length of animation
  ...
  var radiusScale = d3.scale.linear().domain([0, 100]).range([1, 200]);
  var opacityScale = d3.scale.linear().domain([0, 100]).range([0, 1]);

  var drawProbability = d3.scale.linear().domain([0, 100]).range([0, 0.01]);
